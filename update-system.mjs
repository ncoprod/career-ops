...
async function apply() {
  ...
  if (!reviewed) {
    throw new Error('Refusing to apply update without review gate. Run: node update-system.mjs apply --reviewed [--install-deps]');
  }
  ...
  try {
    ...
    // 5. Optional dependency install. Disabled by default so freshly fetched
    // code is never executed automatically during update apply.
    if (installDeps) {
      try {
        execFileSync('npm', ['install', '--ignore-scripts', '--silent'], {
          cwd: ROOT,
          timeout: 120000,
      ...
      console.log('Dependency install skipped. Run npm install --ignore-scripts after reviewing changes if needed.');
    }
